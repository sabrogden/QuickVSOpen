...
namespace QuickVSOpen
{
	public static class Log
	{
		...
		private static void OnMessage(Level level, string format, object[] args)
		{
			...
			if(mPrefix.Length > 0)
			{
				formattedLine = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss.fffffff") + " " + mPrefix + " (" + levelName + "): " + indent + message + "\n";
			}
			else
			{
				formattedLine = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss.fffffff") + " " + levelName + ": " + indent + message + "\n";
			}
